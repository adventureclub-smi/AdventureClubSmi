  ...
  return (
    <group ref={groupRef} rotation={[-0.55, 0, 0]} position={[0, -1.2, 0]}>
      <mesh geometry={geometry}>
        <meshBasicMaterial color="#008668" wireframe transparent opacity={0.32} />
      </mesh>
    </group>
  );
}
...
export default function HeroScene({
  ...
  return (
    <Canvas
      // Capped noticeably lower than a typical scene, and antialiasing
      // dropped entirely — this is a thin, 32%-opacity wireframe sitting on
      // top of a decoding video with its own scroll-driven transform, so
      // every extra sample here is GPU work fighting the video for the same
      // frame budget. MSAA in particular barely reads at this opacity.
      dpr={[1, isMobile ? 1 : 1.3]}
