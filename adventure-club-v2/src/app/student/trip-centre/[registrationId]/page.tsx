...
export default function StudentTripCentre() {
  const { registrationId } = useParams();
  const storageKey = `trip-checklist-${registrationId}`;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/student/trip-centre/${registrationId}`);
        ...
        const announcementRes = await fetch(
          `/api/student/trip-announcements/${data.trek.id}`
        );
  ...
  const meetingTime = registration?.trek?.meetingTime;
  ...
  function toggleItem(item: string) {
    ...
    if (checkedItems.includes(item)) {
      updated = checkedItems.filter((i) => i !== item);
    } else {
  ...
  const totalItems = useMemo(() => {
    ...
    return (
      registration.trek.requiredItems.length +
      registration.trek.optionalItems.length
    );
  }, [registration]);
  ...
  if (loading)
    return <div className={styles.loading}>Loading trip centre...</div>;

  if (!registration)
    return <div className={styles.loading}>Trip Centre unavailable.</div>;
    ...
    <div className={styles.container}>
      ...
      >
        <div className={styles.heroContent}>
          <p className={styles.eyebrow}>Student Trip Centre</p>
          <h1>{trek.title}</h1>
          <p>
          ...
          <div className={styles.infoGrid}>
            ...
            <section className={styles.card}>
              ...
              <div className={styles.weatherIcon}>
                <Umbrella size={36} strokeWidth={1.75} />
                <strong>Temp TBA</strong>
              </div>
